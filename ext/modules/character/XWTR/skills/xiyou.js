import { lib, game, ui, get, ai, _status } from "../../../../../../../noname.js";

/** @type { importCharacterConfig['skill'] } */

export const xiyouSkills = {
	//西游释厄传
	"xjzh_xyj_tianhuo": {
		enable: "phaseUse",
		init(player, skill) {
			player.addMark(skill, 3, false);
			player.update();
			game.xjzh_playAudio('xjzh_xyj_tianhuochuchang');
		},
		mark: true,
		marktext: "火",
		intro: {
			name: "天火",
			content: "本局游戏可发动#次",
		},
		audio: "ext:仙家之魂/audio/skill:2",
		filterTarget(card, player, target) {
			return get.xjzh_nearbyRole(player).includes(target);
		},
		filterCard(card, player, target) {
			return get.suit(card) == "diamond";
		},
		selectCard() {
			let player = get.player(), cards = player.getCards('he', { suit: "diamond" });
			return [1, Math.max(1, cards.length)];
		},
		position: 'he',
		lose: false,
		filter(event, player) {
			if (!player.countCards('he', { suit: "diamond" })) return false;
			if (!player.hasMark("xjzh_xyj_tianhuo")) return false;
			return true;
		},
		mod: {
			cardUsable(card, player, num) {
				let history = player.getHistory('useCard', evt => evt.card && get.name(evt.card) == "xjzh_card_hunyuandan");
				if (history.length && get.type(card) == "basic") return true;
				return num;
			},
		},
		async content(event, trigger, player) {
			player.removeMark("xjzh_xyj_tianhuo", 1, false);
			await event.targets[0].gain(event.cards, player, "draw");
			let thcards = event.cards.slice(0), reverseBool = false;

			let targets = get.players(player).filter(target => target != player);
			if (event.targets[0] == player.getPrevious()) {
				targets.reverse();
				reverseBool = true;
			}
			for await (let target of targets) {
				game.delay();
				let res = get.damageEffect(target, player, target, 'fire');
				const result = await target.chooseCard(thcards.length + 1, { suit: "diamond" })
					.set('ai', card => {
						if (get.player().hasSkillTag('nofire')) return -1;
						if (get.event().res >= 0) return 6 - get.value(card);
						if (get.type(card) != 'basic') return 10 - get.value(card);
						return 8 - get.value(card);
					})
					.set("prompt", `〖天火〗：选择${get.translation(thcards.length + 1)}张♦牌交给${get.translation(reverseBool == true ? target.getPrevious() : target.getNext())}，否则受到${get.translation(thcards.length)}点火焰伤害`)
					.set('res', res)
					.forResult();
				if (result?.cards) {
					let goalTarget = reverseBool == true ? target.getPrevious() : target.getNext()
					target.line(goalTarget, 'fire');
					goalTarget.gain(result.cards, target, "draw");
					thcards = result.cards.slice(0);
				} else {
					target.damage(player, thcards.length, "nocard", "fire");
					break;
				}
			}
		},
		ai: {
			order: 1,
			result: {
				player(player, target, card) {
					if (player.hasUnknown(2)) return 0;
					let num = 0, eff = 0, players = game.filterPlayer();
					for (let target of players) {
						if (get.damageEffect(target, player, target, 'fire') >= 0) { num = 0; continue };
						let shao = false;
						num++;
						if (target.countCards('h', card => {
							if (get.suit(card) != 'diamond') {
								return get.value(card) < 10;
							}
							return get.value(card) < 8;
						}) < num) shao = true;
						if (shao) {
							eff -= 4 * (get.realAttitude || get.attitude)(player, target);
							num = 0;
						}
						else eff -= num * (get.realAttitude || get.attitude)(player, target) / 4;
					}
					if (eff < 4) return 0;
					return eff;
				},
			},
		},
	},
	"xjzh_xyj_dongcha": {
		trigger: {
			player: "phaseDrawBegin",
		},
		audio: "ext:仙家之魂/audio/skill:2",
		forced: true,
		locked: true,
		priority: 1,
		filter(event, player) {
			return !event.numFixed;
		},
		async content(event, trigger, player) {
			trigger.num += 2;
		},
		ai: {
			viewHandcard: true,
			skillTagFilter(player, tag, arg) {
				if (tag == 'viewHandcard') {
					if (player == arg) return false;
					return true;
				};
			},
		},
	},
	"xjzh_xyj_ruyi": {
		trigger: {
			global: "damageAfter",
		},
		forced: true,
		locked: true,
		priority: 2,
		audio: "ext:仙家之魂/audio/skill:2",
		filter(event, player) {
			if (!game.hasNature(event, "fire")) return false;
			if (event.getParent().name == "xjzh_xyj_tianhuo") return false;
			if (event.player != player) return event.source == player;
			return true;
		},
		mod: {
			ignoredHandcard(card, player) {
				if (!get.is.playerNames(player, 'xjzh_xyj_sunwukong')) return;
				let cards = [
					"xjzh_card_tianganghuo",
					"xjzh_card_hunyuandan",
					"xjzh_card_huoyundao",
					"xjzh_card_dingshenzhou",
					"xjzh_card_zhaoyaojing"
				];
				if (cards.includes(get.name(card, player))) return true;
			},
			canBeGained(card, player, target) {
				if (!get.is.playerNames(target, 'xjzh_xyj_sunwukong')) return;
				let cards = [
					"xjzh_card_tianganghuo",
					"xjzh_card_hunyuandan",
					"xjzh_card_dingshenzhou",
					"xjzh_card_zhaoyaojing"
				];
				if (cards.includes(get.name(card, target))) return false;
			},
			canBeDiscarded(card, player, target) {
				if (!get.is.playerNames(target, 'xjzh_xyj_sunwukong')) return;
				let cards = [
					"xjzh_card_tianganghuo",
					"xjzh_card_hunyuandan",
					"xjzh_card_dingshenzhou",
					"xjzh_card_zhaoyaojing"
				];
				if (cards.includes(get.name(card, target))) return false;
			},
			cardDiscardable(card, player, name) {
				if (!get.is.playerNames(player, 'xjzh_xyj_sunwukong')) return;
				let cards = [
					"xjzh_card_tianganghuo",
					"xjzh_card_hunyuandan",
					"xjzh_card_dingshenzhou",
					"xjzh_card_zhaoyaojing"
				];
				if (cards.includes(get.name(card, player))) return false;
			},
			aiValue(player, card, num) {
				if (!get.is.playerNames(player, 'xjzh_xyj_sunwukong')) return;
				let cards = [
					"xjzh_card_tianganghuo",
					"xjzh_card_hunyuandan",
					"xjzh_card_huoyundao",
					"xjzh_card_dingshenzhou",
					"xjzh_card_zhaoyaojing"
				];
				if (cards.includes(get.name(card, player))) return num + 10;
			},
		},
		getIndex(event) {
			return event.num || 1;
		},
		async content(event, trigger, player) {
			let cards = ["xjzh_card_tianganghuo", "xjzh_card_hunyuandan", "xjzh_card_huoyundao", "xjzh_card_dingshenzhou", "xjzh_card_zhaoyaojing"].randomGet();
			player.gain(game.createCard(cards, "red", get.rand(1, 13)), "gain2", "log", player)._triggered = null;
		},
		ai: {
			expose: 0.5,
			threaten: 1.5,
			effect: {
				target(card, player, target) {
					if (get.tag(card, "fireDamage") && target.hasSkill("xjzh_xyj_ruyi")) return [1, 0.5];
				},
			},
			result: {
				player(player, target, card) {
						if (get.tag(card, "fireDamage") && player.hasSkill("xjzh_xyj_ruyi")) return 1.5;
					return 1;
				},
			},
		},
	},

};