	...
	items.forEach(function(item) {
		...
		html += '<div class="trade-item" data-item-id="' + item.id + '" style="background:rgba(255,255,255,0.04);border:1px solid ' + rarityColor + '33;border-radius:10px;padding:12px;' + (isSoldOut ? 'opacity:0.5;' : '') + '">' +
			...
			'<div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap;">' +
				'<span style="color:rgba(255,255,255,0.5);font-size:11px;">持有: ' + playerStock + '</span>' +
			...
			'<div style="display:flex;gap:6px;">' +
				'<button class="trade-buy" data-id="' + item.id + '" data-amount="1" ' + (canBuy ? '' : 'disabled style="opacity:0.5;cursor:not-allowed;"') + ' style="flex:1;background:linear-gradient(135deg,#4CAF50,#388E3C);color:#fff;border:none;padding:6px;border-radius:6px;cursor:pointer;font-size:11px;">买入×1</button>' +
				'<button class="trade-buy" data-id="' + item.id + '" data-amount="10" ' + (canBuy && item.stock >= 10 ? '' : 'disabled style="opacity:0.5;cursor:not-allowed;"') + ' style="flex:1;background:linear-gradient(135deg,#4CAF50,#388E3C);color:#fff;border:none;padding:6px;border-radius:6px;cursor:pointer;font-size:11px;">×10</button>' +
				'<button class="trade-sell" data-id="' + item.id + '" data-amount="1" ' + (playerStock > 0 ? '' : 'disabled style="opacity:0.5;cursor:not-allowed;"') + ' style="flex:1;background:linear-gradient(135deg,#FF9800,#F57C00);color:#fff;border:none;padding:6px;border-radius:6px;cursor:pointer;font-size:11px;">卖出×1</button>' +
				'<button class="trade-sell" data-id="' + item.id + '" data-amount="10" ' + (playerStock >= 10 ? '' : 'disabled style="opacity:0.5;cursor:not-allowed;"') + ' style="flex:1;background:linear-gradient(135deg,#FF9800,#F57C00);color:#fff;border:none;padding:6px;border-radius:6px;cursor:pointer;font-size:11px;">×10</button>' +
			'</div>' +
		'</div>';
	});
