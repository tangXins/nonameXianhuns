...
import checkNoFire from './other.js';
...
class updates {
    ...
    async checkCoreUpdate() {
        if (typeof get.coreInfo !== "function") {
            alert('你的客户端版本过低，请升级客户端版本');
            return false;
        }
        else {
            let coreInfo = get.coreInfo();
            if (coreInfo?.[1] < 98) {
                let str = `
                \n你的客户端内核版本为${coreInfo[1]}，请升级至98及以上版本；
                \n通常此提示出现的原因是你的安卓版本过低，或者你的启动器版本过低，请更新安卓版本或启动器；
                \n如你无法更新安卓版本或更新启动器后此提示仍然出现，请更新webview，更新方法各大无名杀群均有，善用搜索；
                `;
                alert(str);
                return false;
            }
    ...
    async checkBannedExtension() {
        let result = await checkNoFire.checkBanned();
        if (result === true) {
            game.removeExtension("仙家之魂");
            throw new Error('你已安装与《仙家之魂》不兼容的扩展，已为你自动删除《仙家之魂》扩展');
        };
    };
    showUpdateLog() {
        try {
            ...
            caption = `<b style="font-size: 26px;">仙家之魂${version}更新档说明</b><br>`;
            let biaoti = Object.keys(changeLog);

            for (const i of biaoti) {
                if (!changeLog[i].length) continue;
                let li = document.createElement('li');
                li.innerHTML = `<br><b>${i}</b>`;
                li.style.cssText = 'text-align: center; list-style-type: none; font-size: 20px; font-weight: bold;';
                ul.appendChild(li);
                for (const j of changeLog[i]) {
                    li = document.createElement('li');
                    li.style.cssText = 'line-height: 1.5;';
                    li.innerHTML = j;
                    ul.appendChild(li);
                }
            }
