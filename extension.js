...
game.import("extension",function(lib,game,ui,get,ai,_status){
	return {
    	...
    	/*if (typeof game.shijianCreateProgress != 'function') {
			...
			/*let originGain=lib.element.content.gain;
			lib.element.content.gain=function(){
                originGain.apply(this,arguments);
                for(var i=0;i<cards.length;i++){
					var info=get.info(cards[i],false);
					if(info.onGain&&(!info.filterGain||info.filterGain(card,player))){
						if(Array.isArray(info.onGain)){
							for(var i=0;i<info.onGain.length;i++){
								var next=game.createEvent('gain_'+cards[i].name);
								next.setContent(info.onGain[i]);
								next.player=player;
								next.card=cards[i];
							}
						}
							...
							var next=game.createEvent('gain_'+cards[i].name);
							next.setContent(info.onGain);
							next.player=player;
							next.card=cards[i];
						}
						if(info.gainDelay!='false') game.delayx();
					}
				}
            };*/
			//判断标记是否为唯一最多
